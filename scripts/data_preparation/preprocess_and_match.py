#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# esempio: python3 /media/naza/SBT2025/pipeline/scripts/data_preparation/preprocess_and_match.py -e 1 
...
def main():
    ...
    # Paths
    DATA_ROOT = Path(__file__).parent.parent.parent # Modificato: Risale di 3 livelli (scripts/data_preparation/ -> MDE-GitHub/)
    
    # Adattamento dei percorsi per la struttura MDE-GitHub
    DATA_DIR = DATA_ROOT / "data" 
    VIDEO_PATH = DATA_DIR / "raw" / "video_raw" / EXP["video_raw"] 
    MCAP_PATH = DATA_DIR / "raw" / "mcap" / EXP["mcap"] # Assume che MCAP sia nella sua sottocartella in raw/
    LABELS_DIR = DATA_DIR / "raw" / "label" / EXP["label"] # Assume labels in raw/
    
    # Outputs
    PREPROCESSED_DIR = DATA_DIR / "preprocessed"
    FRAMES_OUTPUT = PREPROCESSED_DIR / f"frames_{EXP['name']}"
    GPS_CSV_OUTPUT = PREPROCESSED_DIR / f"gps_{EXP['name']}.csv"
    VIDEO_1FPS_OUTPUT = PREPROCESSED_DIR / f"video_1fps_{EXP['name']}.mp4"
    ...
    camera_position_path = PREPROCESSED_DIR / "camera_position.csv"
    anchor_points_path = PREPROCESSED_DIR / "anchor_points.csv"
    ...
    # ... (restante logica di estrazione frame, GPS e matching)
    # =========================================================================
